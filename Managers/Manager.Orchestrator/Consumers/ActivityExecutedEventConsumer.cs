...
public class ActivityExecutedEventConsumer : IConsumer<ActivityExecutedEvent>
{
    ...
    public async Task Consume(ConsumeContext<ActivityExecutedEvent> context)
    {
        ...
        _logger.LogInformationWithHierarchy(eventContext,
            "Processing ActivityExecutedEvent. Status: {Status}, Duration: {Duration}ms, EntitiesProcessed: {EntitiesProcessed}",
            activityEvent.Status, activityEvent.Duration.TotalMilliseconds, activityEvent.EntitiesProcessed);
        ...
        try
        {
            ...
            if (nextSteps.Count == 0)
            {
                ...
                _logger.LogInformationWithHierarchy(eventContext,
                    "Flow branch termination detected and processed for executed activity. Duration: {Duration}ms",
            ...
            _logger.LogInformationWithHierarchy(eventContext,
                "Successfully processed ActivityExecutedEvent. NextSteps: {NextStepCount}, PublishedCommands: {PublishedCommands}, Duration: {Duration}ms",
        ...
        catch (Exception ex)
        {
            ...
            _logger.LogErrorWithHierarchy(eventContext, ex,
                "Error processing ActivityExecutedEvent. Duration: {Duration}ms, ErrorType: {ErrorType}",
    ...
    private async Task HandleFlowBranchTerminationAsync(ActivityExecutedEvent activityEvent)
    {
        ...
        // Create hierarchical context for flow termination
        ...
        try
        {
            ...
            var key = _rawCacheService.GetProcessorCacheKey(activityEvent.ProcessorId, activityEvent.OrchestratedFlowId, activityEvent.CorrelationId, activityEvent.ExecutionId, activityEvent.StepId, activityEvent.PublishId);

            await _rawCacheService.RemoveAsync(mapName, key);

            _logger.LogInformationWithHierarchy(terminationContext,
                "Deleted cache processor data for flow branch termination. MapName: {MapName}, Key: {Key}",
                mapName, key);
    ...
    private async Task ProcessSingleNextStepAsync(ActivityExecutedEvent activityEvent, Guid nextStepId, OrchestrationCacheModel orchestrationData)
    {
        ...
        // Get next step entity (declare outside try block for catch block access)
        ...
        // Create Layer 5 hierarchical context for next step processing - used throughout the method
        ...
        try
        {
            ...
            if (!shouldExecuteStep)
            {
                _logger.LogInformationWithHierarchy(stepContext,
                    ...
                    nextStepEntity.EntryCondition, activityEvent.Status);
                return;
            }
            ...
            // Step 4.4: Compose and publish ExecuteActivityCommand with WorkflowId
            var command = new ExecuteActivityCommand
            {
                OrchestratedFlowId = activityEvent.OrchestratedFlowId,
                WorkflowId = activityEvent.WorkflowId, // ✅ Include WorkflowId from event
                CorrelationId = activityEvent.CorrelationId,
                StepId = nextStepId,
                ProcessorId = nextStepEntity.ProcessorId,
            ...
            // Layer 5 log - Workflow step transition
            _logger.LogInformationWithHierarchy(stepContext,
                "Workflow step transition. FromStepId: {FromStepId}, WorkflowPhase: {WorkflowPhase}, AssignmentCount: {AssignmentCount}, TransitionDuration: {TransitionDuration}ms",
                activityEvent.StepId, "StepTransition", assignmentModels.Count, publishStopwatch.ElapsedMilliseconds);
    ...
    private async Task CopyCacheProcessorDataAsync(ActivityExecutedEvent activityEvent, Guid nextStepId, Guid destinationProcessorId, Guid publishId)
    {
        ...
        try
        {
            ...
            var destinationKey = _rawCacheService.GetProcessorCacheKey(destinationProcessorId, activityEvent.OrchestratedFlowId, activityEvent.CorrelationId, activityEvent.ExecutionId, nextStepId, publishId);
            ...
            if (!string.IsNullOrEmpty(sourceData))
            {
                ...
                _logger.LogInformationWithHierarchy(copyContext,
                    ...
                    destinationMapName, destinationKey, sourceData.Length);

                _logger.LogDebugWithHierarchy(copyContext,
                    "Copied cache processor data. Source: {SourceMapName}:{SourceKey} -> Destination: {DestinationMapName}:{DestinationKey}",
                    sourceMapName, sourceKey, destinationMapName, destinationKey);
            }
            else
            {
                _logger.LogWarningWithHierarchy(copyContext,
                    "No source data found to copy. Source: {SourceMapName}:{SourceKey}",
        ...
        catch (Exception ex)
        {
            ...
            _logger.LogErrorWithHierarchy(copyContext, ex,
                "Failed to copy cache processor data. SourceProcessorId: {SourceProcessorId}, DestinationProcessorId: {DestinationProcessorId}",
    ...
    private async Task DeleteSourceCacheDataAsync(ActivityExecutedEvent activityEvent)
    {
        ...
        try
        {
            ...
            _logger.LogDebugWithHierarchy(deleteContext,
                "Deleted source cache data. Source: {SourceMapName}:{SourceKey}",
                sourceMapName, sourceKey);
        }
        catch (Exception ex)
        {
            ...
            _logger.LogErrorWithHierarchy(deleteContext, ex,
                "Failed to delete source cache data");
