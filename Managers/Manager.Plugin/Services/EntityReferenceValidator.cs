...
public class EntityReferenceValidator : IEntityReferenceValidator
{
    ...
    public async Task ValidateEntityCanBeDeleted(Guid entityId)
    {
        ...
        try
        {
            ...
            if (hasReferences)
            {
                var message = $"Cannot delete Plugin entity {entityId}: it is referenced by one or more Assignment entities";
        ...
        {
            var message = $"Cannot validate Plugin entity {entityId} for deletion: validation service unavailable";
