using Manager.Plugin.Consumers;
...
// Add MongoDB
builder.Services.AddMongoDb<IPluginEntityRepository, PluginEntityRepository>(builder.Configuration, builder.Configuration["MongoDB:DatabaseName"]!);

// Add MassTransit Bus Provider
builder.Services.AddMassTransitBusProvider(builder.Configuration,
    typeof(CreatePluginCommandConsumer),
    typeof(UpdatePluginCommandConsumer),
    typeof(DeletePluginCommandConsumer),
    typeof(GetPluginQueryConsumer));
...
try
{
    ...
    logger.LogInformation("Starting PluginManager API");
    app.Run();
}
catch (Exception ex)
{
